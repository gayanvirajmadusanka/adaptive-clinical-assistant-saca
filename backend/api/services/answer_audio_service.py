...
def _build_question_wp_keywords() -> dict:
    """
    Build a map from question_id to the set of valid Warlpiri keywords for that question.
    Used to restrict DTW matching to only the words that are actual answer options,
    preventing the recogniser from confusing answer words with symptom keywords.
    """
    mapping = {}

    # mandatory questions: derive from each option's text_wp
    for question in _QUESTIONS['mandatory']:
        qid = question['id']
    ...
    # all yes/no questions (symptom follow-ups + confirmation)
...
def _resolve_warlpiri(tmp_path: str, question_id: str, base: dict) -> dict:
    ...
    if not answer_id:
        return _unrecognised(base, f'Wangkaya nyangu, kala japi-wangu: {best_keyword}',
