export const translations = {
  en: {
    input_title: 'Hi, Tell us how you feel?',
    input_subtitle: 'You can speak, type or use the body map to describe your symptoms.',

    text: 'WRITE',
    voice: 'SPEAK',
    body: 'BODY',
    show: 'SHOW',
    ok: 'OK',
    cancel: 'Cancel',

    home: 'Home',
    language: 'Language',

    select_language: 'Select Language',
    english: 'English',
    warlpiri: 'Warlpiri',

    confirm: 'Confirm',
    change_language: 'Change language?',
    yes: 'YES',
    no: 'NO',

    text_input_title: 'Write',
    speak_option: 'Speak',
    speak_hint: 'Click on mic to record voice',
    recording_hint: 'Recording... tap to stop',
    processing_voice: 'Processing your voice...',
    text_input_question: 'What can I help with?',
    text_placeholder: 'Describe your symptoms and press Continue',
    continue: 'Continue',
    back: 'Back',
    submit: 'Submit',
    question: 'Question',

    detected_title: 'Detected Symptoms',
    detected_question: 'Does this match you?',
    detected_speak: 'Detected symptoms are headache, fever, body pain, and tiredness.',

    tell_us_more: 'Tell Us More',
    pain_level: 'PAIN LEVEL',
    pain_question: 'How bad is your pain?',

    none: 'None',
    little: 'A little',
    moderate: 'Moderate',
    very_bad: 'Very bad',
    unbearable: 'Unbearable',

    how_long: 'HOW LONG',
    duration_question: 'How long have you felt this way?',

    today: 'Today',
    yesterday: 'Yesterday',
    two_three_days: '2–3 days',
    about_week: 'About a week',
    more_than_week: 'More than a week',

    loading: 'LOADING',
    loading_wait: 'This may take a few minutes please wait...',
    loading_symptoms: 'Checking your Symptoms...',
    loading_severity: 'Checking your Severity...',

    result: 'Final Results',
    recommendations: 'Recommendations',
    symptoms: 'Symptoms',
    severity: 'Severity',
    mild_label: 'Severity: Mild',
    moderate_label: 'Severity: Moderate',
    severe_label: 'Severity: Severe',

    // Symptoms from symptom_map.json
    headache: 'Headache',
    dizziness: 'Dizziness',
    loss_of_consciousness: 'Loss of consciousness',
    stiff_neck: 'Stiff neck',
    chest_pain: 'Chest pain',
    shortness_breath: 'Shortness breath',
    cough: 'Cough',
    abdominal_pain: 'Abdominal pain',
    nausea: 'Nausea',
    vomiting: 'Vomiting',
    diarrhea: 'Diarrhea',
    blood_stool: 'Blood stool',
    arm_pain: 'Arm pain',
    arm_weakness: 'Arm weakness',
    swelling_arms: 'Swelling arms',
    jaw_pain: 'Jaw pain',
    runny_nose: 'Runny nose',
    sneezing: 'Sneezing',
    ear_pain: 'Ear pain',
    eye_pain: 'Eye pain',
    eye_itching: 'Eye itching',
    sore_throat: 'Sore throat',
    back_pain: 'Back pain',
    fever: 'Fever',
    chills: 'Chills',
    fatigue: 'Fatigue',
    weakness: 'Weakness',
    body_pain: 'Body pain',
    swelling_parts_of_body: 'Swelling parts of body',
    rash: 'Rash',
    itchy: 'Itchy',
    bleeding: 'Bleeding',
    dehydration: 'Dehydration',
    blood_urine: 'Blood urine',

    // Existing app aliases
    tiredness: 'Tiredness',
    stomach_pain: 'Stomach pain',
    breathing_problem: 'Breathing problem',

    call_emergency: 'Call Emergency',
    start_again: 'Start Again',

    mild_recommendation_1: 'Rest and drink enough water.',
    mild_recommendation_2: 'Monitor your symptoms.',
    mild_recommendation_3: 'Seek help if symptoms get worse.',

    moderate_recommendation_1: 'Book a health check soon.',
    moderate_recommendation_2: 'Rest and avoid heavy activity.',
    moderate_recommendation_3: 'Get medical advice if pain continues.',

    severe_recommendation_1: 'Seek urgent medical help now.',
    severe_recommendation_2: 'Call emergency services if needed.',
    severe_recommendation_3: 'Do not wait if symptoms are serious.',

    mild_speak: 'Your result is mild. Please rest, drink water, and monitor your symptoms.',
    moderate_speak: 'Your result is moderate. Please book a health check soon and get medical advice if symptoms continue.',
    severe_speak: 'Your result is severe. Please seek urgent medical help now.',

    you_can_treat_this_at_home: 'You can get over the counter medicine.',
    medical_attention_recommended: 'Medical attention is recommended.',
    seek_emergency_help_now: 'Seek help now.',

    about_title: 'Welcome to SACA...',
    about_subtitle: 'Smart Adaptive Clinical Assistant (SACA)',
    about_paragraph:
      'SACA is a smart Mobile app that helps you understand symptoms using AI. Describe issues via text, voice, or body map and get quick insights. It supports early health awareness and is not a replacement for medical advice.',

    body_parts: 'Body Parts',
    male: 'Male',
    female: 'Female',

    missing_information_title: 'Missing information',
    missing_information_message: 'Please describe your symptoms first.',

    detecting_symptoms: 'Detecting Symptoms...',
    analyzing_severity: 'Analyzing Severity...',
    please_wait_symptoms: 'Please wait while we analyze your symptoms...',

    no_symptoms_detected: 'No Symptoms Detected',
    no_symptoms_message_1:'We could not detect any symptoms from your description.',
    no_symptoms_message_2:'Please try describing your symptoms in more detail.',

    // English
    no_answer_title: 'No Answer',
    no_answer_message_1: 'Please select an answer',
    no_answer_message_2: 'Choose one of the options before continuing.',
    question: 'Question',
    of: 'of',
    no_follow_up_questions: 'No follow-up questions found.',

    // English
    done: 'DONE',
    please_wait_severity: 'Please wait while we analyze your severity...',
    error: 'Error',
    severity_error_message: 'Could not get result.',
  },

  wp: {
    input_title: 'Ngarrirni nyuntu, nyarrpa nyinami?',
    input_subtitle: 'Wangkaya, pantirniya, manu milkikarriya nyuntu purrkunypa.',

    text: 'Pantirniya',
    voice: 'Wangkaya',
    body: 'BODY',
    show: 'Milkikarriya',
    ok: 'Yuwayi',
    cancel: 'Lawa',

    home: 'Ngurra',
    language: 'Wangka',

    select_language: 'Wangka ngurrju',
    english: 'Ingliji',
    warlpiri: 'Warlpiri',

    confirm: 'Kurlarda',
    change_language: 'Wangka kutju-kurlu nyampu?',
    yes: 'Yuwayi',
    no: 'Lawa',

    text_input_title: 'Pantirniya',
    speak_option: 'Wangkaya',
    speak_hint: 'Mic-kurra nyampu, wangka record-ki',
    recording_hint: 'Recording... stop-ki mic nyampuju',
    processing_voice: 'Voice nyanyi...',
    text_input_question: 'Nyiya ka nyuntu nyampuju?',
    text_placeholder: 'Ngarrirni nyuntu purrkunypa.',
    continue: 'Yangka',
    back: 'Yankirri',
    submit: 'Yungkurlu',
    question: 'Japi',

    detected_title: 'Purrkunypa nyangu',
    detected_question: 'Nyampu nyuntu-kurra?',
    detected_speak: 'Purrkunypa nyangu Walpawalpa, Rdurrurlpu, Palka purrkunypa, manu Mata.',

    tell_us_more: 'Ngarrirni nyuntu panu',
    pain_level: 'Pina Level',
    pain_question: 'Nyuntu pina ngurrju?',

    none: 'Wangu',
    little: 'Jintangka',
    moderate: 'Marda',
    very_bad: 'Pina Wiri',
    unbearable: 'Pina Wiri Manu',

    how_long: 'Nyarrpara',
    duration_question: 'Nyarrpara-jangka nyuntu nyampu?',

    today: 'Kurdu-jinta',
    yesterday: 'Yurrpu-jinta',
    two_three_days: '2–3 karnta',
    about_week: 'Week-jinta',
    more_than_week: 'Week-jinta jangka',

    loading: 'Yirrarni',
    loading_wait: 'Nyampu may take few minutes, please wait...',
    loading_symptoms: 'Purrkunypa nguntupinyi nyuntu-nyangu....',
    loading_severity: 'Purrkunypa wiri nguntupinyi...',

    result: 'Purrkunypa Nyangu',
    recommendations: 'Ngurrju yimi',
    symptoms: 'Purrkunypa',
    severity: 'Wiri',
    mild_label: 'Wiri: Witapardu',
    moderate_label: 'Wiri: Wiriwiri',
    severe_label: 'Wiri: Wirinyayirni',

    // Symptoms from symptom_map.json
    headache: 'Walpawalpa',
    dizziness: 'Kurruru',
    loss_of_consciousness: 'Tiirnki',
    stiff_neck: 'Nguku kantalkantal',
    chest_pain: 'Mangarli purrkunypa',
    shortness_breath: 'Nguurlnguurlpa',
    cough: 'Kuntulpinyi',
    abdominal_pain: 'Miyalu purrkunypa',
    nausea: 'Ngarlungarlu',
    vomiting: 'Yurlkulyu',
    diarrhea: 'Jinirrpa',
    blood_stool: 'Jinirrpa jaarlkurlu',
    arm_pain: 'Kilpirli purrkunypa',
    arm_weakness: 'Kilpirli rampaku',
    swelling_arms: 'Kilpirli kirakarrimi',
    jaw_pain: 'Wirlki purrkunypa',
    runny_nose: 'Mulyu rdulpu',
    sneezing: 'Jirrjinti',
    ear_pain: 'Langa purrkunypa',
    eye_pain: 'Milpa purrkunypa',
    eye_itching: 'Milpa yayirri',
    sore_throat: 'Waninja kiri',
    back_pain: 'Pawiyi purrkunypa',
    fever: 'Rdurrurlpu',
    chills: 'Mirrmirr',
    fatigue: 'Mata',
    weakness: 'Rampaku',
    body_pain: 'Palka purrkunypa',
    swelling_parts_of_body: 'Kirakarrimi',
    rash: 'Mardalmardal',
    itchy: 'Yayirri',
    bleeding: 'Ngamanjimanji',
    dehydration: 'Nantu',
    blood_urine: 'Minjinpa jaarlkurlu',

    // Existing app aliases
    tiredness: 'Mata',
    stomach_pain: 'Miyalu purrkunypa',
    breathing_problem: 'Nguurlnguurlpa',

    call_emergency: 'Jalangu yardajirri warrirni.',
    start_again: 'Pina Yangka',

    mild_recommendation_1: 'Rest manu water nyinyi.',
    mild_recommendation_2: 'Purrkunypa nyanyi.',
    mild_recommendation_3: 'Purrkunypa wiri-jangka help japirli.',

    moderate_recommendation_1: 'Health check book soon.',
    moderate_recommendation_2: 'Rest manu heavy work lawa.',
    moderate_recommendation_3: 'Pain continue-jangka doctor yimi japirli.',

    severe_recommendation_1: 'Urgent medical help japirli now.',
    severe_recommendation_2: 'Emergency services call if needed.',
    severe_recommendation_3: 'Purrkunypa serious-jangka wait lawa.',

    mild_speak: 'Nyuntu result mild. Rest manu water nyinyi.',
    moderate_speak: 'Nyuntu result moderate. Health check book soon.',
    severe_speak: 'Nyuntu result severe. Urgent medical help japirli now.',

    you_can_treat_this_at_home: 'Mirrijini nyuntu payi-ma-ni.',
    medical_attention_recommended: 'Ngangkayikurra yanta.',
    seek_emergency_help_now: 'Jalangu yardajirri warrirni.',

    about_title: 'Nyampu nyinami SACA-kurra...',
    about_subtitle: 'Smart Adaptive Clinical Assistant (SACA)',
    about_paragraph:
      'SACA ka smart mobile app nyuntu-kurra. Ngula nyuntu symptoms yimi (text), wangka (voice), manu body map-kurlu nyampu yimi. AI ka nyampu nyanyi manu nyuntu quick kulini symptoms. Ngula health early kuju-kuju manu nyuntu ngurrju kulini. Ngula doctor manu medical advice replacement nyampuju lawa.',

    body_parts: 'Palka Yimi',
    male: 'Wati',
    female: 'Karnta',

    missing_information_title: 'Yimi manca.',
    missing_information_message: 'Nyuntu kuja symptom wangka ngurrju-juku.',

    detecting_symptoms: 'Purrkunypa nyanganyi...',
    analyzing_severity: 'Purrkunypa wiri nyanganyi...',
    please_wait_symptoms: 'Wait-ki. Ngula nyuntu purrkunypa nyanganyi.',

    
    no_symptoms_detected: 'Purrkunypa lawa nyangu',
    no_symptoms_message_1:'Ngula purrkunypa lawa nyangu nyuntu yimi-jangka.',
    no_symptoms_message_2:'Nyuntu purrkunypa yimi panu-piya ngarrirni.',

    // Warlpiri
    no_answer_title: 'Japi lawa',
    no_answer_message_1: 'Yuwayi manu lawa milkikariya',
    no_answer_message_2: 'Jinta milarni, yantarla-wangu',
    question: 'Japi',
    of: 'of',
    no_follow_up_questions: 'Japi-kari lawa.',

    // Warlpiri
    done: 'Palya',
    please_wait_severity: 'Wait-ki. Ngula nyuntu purrkunypa wiri nyanganyi.',
    error: 'Error',
    severity_error_message: 'Result lawa nyangu.',
  },
};