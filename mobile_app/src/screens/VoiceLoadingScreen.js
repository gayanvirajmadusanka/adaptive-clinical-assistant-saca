...
// shows fast circular loading progress like LoadingScreen,
// then opens DetectedSymptomsVoiceScreen with detected symptoms and backend audio response.
...
export default function VoiceLoadingScreen() {
  ...
  const isWarlpiri = selectedLanguage === 'wp';
  ...
  async function sendAudioToApi() {
    try {
      ...
      const audioBase64 = await readAudioFileAsBase64(
        String(audio_uri)
      );
