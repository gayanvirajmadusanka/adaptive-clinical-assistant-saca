"""
Tests for backend_release/speech/audio_warlpiri.py
Adapted from backend/tests/test_audio_warlpiri.py.
Uses python_speech_features + scipy instead of librosa.
"""
...
from backend_release_android.speech.audio_warlpiri import (
    KEYWORD_MFCC,
    KEYWORD_SYMPTOM_MAP,
    SAMPLE_RATE,
    DTW_THRESHOLD,
...
class TestModuleLoading(unittest.TestCase):
    ...
    def test_all_50_keywords_loaded(self):
        self.assertGreaterEqual(len(_KEYWORD_REFS), 50)
...
class TestConfidenceScoring(unittest.TestCase):
    ...
    def test_confidence_is_float(self):
        self.assertIsInstance(_distance_to_confidence(100.0), float)
...
class TestRecognizeFunction(unittest.TestCase):
    ...
    def test_result_has_recognized_key(self):
        import tempfile
    ...
    def test_fever_recording_returns_dict(self):
        ...
        if not os.path.exists(fever_wav):
            self.skipTest("fever_wp.wav not available")
...
class TestKeywordMFCCIntegrity(unittest.TestCase):
    ...
    def test_all_symptom_keywords_present(self):
        ...
        for keyword in expected:
            self.assertIn(keyword, KEYWORD_MFCC,
                f"Symptom keyword '{keyword}' missing from keyword_mfcc_android.json")
    ...
    def test_all_answer_keywords_present(self):
        ...
        for keyword in expected:
            self.assertIn(keyword, KEYWORD_MFCC,
                f"Answer keyword '{keyword}' missing from keyword_mfcc_android.json")
    ...
    def test_total_keyword_count_at_least_50(self):
        self.assertGreaterEqual(len(KEYWORD_MFCC), 50)
